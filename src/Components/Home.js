...
function Home(props) {
  ...
  const [selectedPrices, setSelectedPrices] = useState(
    props.location.state ? props.location.state.prices : []
  );
  const [sortBy, setSortBy] = useState(
    props.location.state ? props.location.state.sortBy : 'nameAsc'
  );
  const [view, setView] = useState(
    props.location.state ? props.location.state.view : 'grid'
  );
  ...
  const columns: GridColDef[] = [
    ...
    { field: 'time', headerName: 'Last Updated', width: 300 },
  ];
  ...
  function handleRowSelect(e) {
    history.push({
      pathname: `/ticket/${e.row.ticket_number}`,
      state: { view: view, prices: selectedPrices, sortBy: sortBy },
    });
  }
